  ...
  return (
    <div id="top" className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section id="skills" className="border-y border-border bg-secondary/40 px-6 py-24 sm:px-10 lg:px-16"><div className="mx-auto max-w-6xl"><SectionLabel text="03 / Skills & GitHub" /><div className="mt-10 flex flex-wrap gap-x-6 gap-y-4">{skills.map((skill) => <a key={skill} href={`${site.github}/search?q=${encodeURIComponent(skill)}&type=repositories`} target="_blank" rel="noreferrer" className="group inline-flex items-center gap-2 text-sm transition hover:text-primary"><span className="size-1.5 rounded-full bg-border transition group-hover:bg-primary" aria-hidden="true" />{skill}</a>)}</div><a href={site.github} target="_blank" rel="noreferrer" className="mt-12 flex max-w-xl items-center justify-between border-t border-border pt-5 text-lg font-medium transition hover:text-primary"><span><Github className="mr-3 inline" size={19} />See my open-source work</span><ArrowUpRight size={19} /></a></div></section>
