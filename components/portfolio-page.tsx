...
const skills = [
  ["Python", "python", "3776AB"], ["SQL", "databricks", "FF3621"], ["FastAPI", "fastapi", "009688"], ["Node.js", "nodedotjs", "5FA04E"], ["TypeScript", "typescript", "3178C6"], ["C++", "cplusplus", "00599C"], ["Streamlit", "streamlit", "FF4B4B"], ["TensorFlow", "tensorflow", "FF6F00"], ["PyTorch", "pytorch", "EE4C2C"], ["Scikit-learn", "scikitlearn", "F7931E"], ["Hugging Face", "huggingface", "FFD21E"], ["OpenCV", "opencv", "5C3EE8"], ["RAG", "", ""], ["Agentic AI", "", ""], ["Prompt Engineering", "", ""], ["LangChain", "langchain", "1C3C3C"], ["LangGraph", "", ""], ["Vector Databases", "", ""], ["Memory Engineering", "", ""], ["MLOps", "", ""], ["Git", "git", "F05032"], ["GitHub Actions", "githubactions", "2088FF"], ["Docker", "docker", "2496ED"], ["CI/CD", "", ""], ["VS Code", "visualstudiocode", "007ACC"], ["Jupyter Notebook", "jupyter", "F37626"], ["Google Colab", "googlecolab", "F9AB00"], ["UiPath", "uipath", "FA4616"],
] as const
  ...
  return (
    <div id="top" className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section id="skills" className="border-y border-border bg-secondary/40 px-6 py-24 sm:px-10 lg:px-16"><div className="mx-auto max-w-6xl"><SectionLabel text="03 / Skills & GitHub" /><div className="mt-10 flex flex-wrap gap-x-6 gap-y-4">{skills.map(([skill, icon, color]) => <a key={skill} href={`${site.github}/search?q=${encodeURIComponent(skill)}&type=repositories`} target="_blank" rel="noreferrer" className="group inline-flex items-center gap-3 text-sm transition hover:text-primary"><span className="flex size-8 items-center justify-center rounded-lg border border-border bg-background/70 p-1.5 transition group-hover:border-primary/50">{icon ? <img src={`https://cdn.simpleicons.org/${icon}/${color}`} alt="" className="size-full object-contain" /> : <span className="font-mono text-[10px] font-semibold text-primary">AI</span>}</span>{skill}</a>)}</div><a href={site.github} target="_blank" rel="noreferrer" className="mt-12 flex max-w-xl items-center justify-between border-t border-border pt-5 text-lg font-medium transition hover:text-primary"><span><Github className="mr-3 inline" size={19} />See my open-source work</span><ArrowUpRight size={19} /></a></div></section>
