  ...
  return (
    <div id="top" className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section id="projects" className="mx-auto max-w-6xl px-6 py-28 sm:px-10 lg:px-16"><SectionLabel text="02 / Selected projects" /><div className="mt-12 grid gap-x-8 gap-y-12 md:grid-cols-2">{projects.map((project, index) => <motion.a whileHover={{ y: -6 }} transition={{ duration: .25 }} href={project.href} target="_blank" rel="noreferrer" key={project.title} className="group border-t border-border pt-5"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-xs uppercase tracking-[0.16em] text-primary">0{index + 1} · {project.type}</p><h3 className="mt-4 text-2xl font-semibold tracking-tight">{project.title}</h3></div><ArrowUpRight className="text-muted-foreground transition group-hover:text-primary" /></div><p className="mt-5 max-w-lg leading-relaxed text-muted-foreground">{project.text}</p><div className="mt-6 flex flex-wrap items-center gap-4"><p className="font-mono text-xs uppercase tracking-[0.16em] text-foreground/60">{project.metric}</p><span className="ml-auto flex gap-3 text-xs font-medium"><span className="underline underline-offset-4">GitHub</span><a href={project.demo} target="_blank" rel="noreferrer" onClick={(event) => event.stopPropagation()} className="text-primary underline underline-offset-4">Live demo</a></span></div></motion.a>)}</div></section>

        <section id="skills" className="border-y border-border bg-secondary/40 px-6 py-24 sm:px-10 lg:px-16"><div className="mx-auto max-w-6xl"><SectionLabel text="03 / Skills & GitHub" /><div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{skills.map((skill) => <a key={skill} href={`${site.github}/search?q=${encodeURIComponent(skill)}&type=repositories`} target="_blank" rel="noreferrer" className="skill-card group flex min-h-24 flex-col justify-between rounded-2xl border border-border/80 bg-background/60 p-4 backdrop-blur-sm transition hover:-translate-y-1 hover:border-primary/50 hover:bg-card"><span className="font-mono text-[10px] uppercase tracking-[0.15em] text-primary">{String(skills.indexOf(skill) + 1).padStart(2, "0")}</span><span className="text-sm font-medium group-hover:text-primary">{skill}</span></a>)}</div><a href={site.github} target="_blank" rel="noreferrer" className="mt-12 flex max-w-xl items-center justify-between border-t border-border pt-5 text-lg font-medium transition hover:text-primary"><span><Github className="mr-3 inline" size={19} />See my open-source work</span><ArrowUpRight size={19} /></a></div></section>
        ...
        <section id="contact" className="bg-primary px-6 py-24 text-primary-foreground sm:px-10 lg:px-16"><div className="mx-auto max-w-6xl"><p className="font-mono text-xs uppercase tracking-[0.2em] opacity-70">05 / Contact</p><h2 className="mt-7 max-w-3xl text-balance text-5xl font-semibold leading-none tracking-[-0.05em] sm:text-7xl">Let&apos;s build something thoughtful.</h2><div className="mt-10 flex flex-wrap items-center gap-5"><a href={`mailto:${site.email}`} className="inline-flex items-center gap-2 border-b border-primary-foreground/40 pb-2 text-lg hover:border-primary-foreground">{site.email} <ArrowUpRight size={18} /></a><a href="tel:+917795429685" className="border-b border-primary-foreground/40 pb-2 text-lg hover:border-primary-foreground">{site.phone}</a><button type="button" onClick={copyEmail} className="inline-flex items-center gap-2 rounded-full border border-primary-foreground/30 px-4 py-2 text-sm transition hover:bg-primary-foreground/10">{copied ? <Check size={16} /> : <Copy size={16} />} {copied ? "Copied" : "Copy email"}</button></div></div></section>
      </main>
      <footer className="flex flex-col gap-5 bg-foreground px-6 py-8 text-background sm:flex-row sm:items-center sm:justify-between sm:px-10 lg:px-16"><span className="font-semibold">Krishna H</span><span className="text-sm opacity-60">Designed and built with care · {new Date().getFullYear()}</span><div className="flex gap-5"><a href={site.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn"><Linkedin size={18} /></a><a href={site.github} target="_blank" rel="noreferrer" aria-label="GitHub"><Github size={18} /></a><a href={site.youtube} target="_blank" rel="noreferrer" aria-label="YouTube" className="text-sm font-medium">YouTube</a></div></footer>
    </div>
  )
}
